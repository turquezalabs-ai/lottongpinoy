...
// --- CONFIG ---
// 1. Real-time Source (GitHub Raw URL)
 $URL_TEMP = 'https://raw.githubusercontent.com/turquezalabs-ai/lottongpinoy-scraper/main/data/results_temp_2d_3d.json';

// 2. Official Archive Source (GitHub Raw URL)
 $URL_MAIN = 'https://raw.githubusercontent.com/turquezalabs-ai/lottongpinoy-scraper/main/data/results.json';
...
// 1. FETCH DATA (Try Temp first, then Main)
 $json_data = null;

log_msg("🌐 Fetching Real-time Data...");
 $response = fetchUrl($URL_TEMP);
if ($response) {
    $json_data = json_decode($response, true);
    log_msg("✅ Loaded: results_temp_2d_3d.json");
}

if (!$json_data) {
    log_msg("⚠️ Temp file empty/failed. Trying Official Data...");
    $response = fetchUrl($URL_MAIN);
    if ($response) {
        $json_data = json_decode($response, true);
        log_msg("✅ Loaded: results.json");
    }
}

if (!$json_data) { log_msg("❌ Error: Could not fetch any data."); exit; }
...
if (!$target) { log_msg("⚠️ No results found for schedule: $schedule"); exit; }
