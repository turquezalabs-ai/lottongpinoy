...
export function initGenerator(allData) {
    ...
    // ==========================================
    // 2. FORCE DEFAULT STATE (The "Click" Trick)
    // ==========================================
    // We find the normal radio button and physically click it via code.
    // This forces the browser to update the UI and triggers the listener above.
    ...
    // NOTE: The Share button listener is now handled by the inline script in generator.php
    // to capture the 800x800 image. You can keep this or remove it if it conflicts.
    ...
    gameSelect?.addEventListener('change', (e) => {
        ...
        // We select normalRadio again inside this scope to ensure it's available
        const currentNormalRadio = document.querySelector('input[name="genMethod"][value="normal"]');

        if (isDigit) {
            if (systemWrapper) systemWrapper.classList.add('hidden');
            if (currentNormalRadio) currentNormalRadio.click();
        } else {
...
function renderOutput(numbers, game) {
    ...
    if (copyBtn) {
        copyBtn.onclick = () => {
            ...
            copyBtn.innerHTML = '<i class="fa-regular fa-copy mr-2"></i>Copied!';
            setTimeout(() => copyBtn.innerHTML = '<i class="fa-regular fa-copy mr-2"></i>Copy', 1500);
        };
    }
}

// ==========================================
// SHARE IMAGE FUNCTION (Fallback)
// ==========================================
